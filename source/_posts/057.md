...
tags: 
    ...
    - 爬虫
categories: 验证码逆向实战
feature: https://cdn.itbob.cn/img/thumbnail/captcha1.png
---
...
![05](https://cdn.itbob.cn/img/article/057/05.png)

`_taN()` 函数是一些 URL，UA 之类的信息，可以写死：

![06](https://cdn.itbob.cn/img/article/057/06.png)

往里跟就可以看到 AES 算法了：

![07](https://cdn.itbob.cn/img/article/057/07.png)
...
![08](https://cdn.itbob.cn/img/article/057/08.png)

![09](https://cdn.itbob.cn/img/article/057/09.png)

解密结果可以看到滑块的图片地址等信息：

![10](https://cdn.itbob.cn/img/article/057/10.png)

## data 生成

接下来就是 `checkInfoTp` 提交验证了，要搞清楚提交的 `data` 是什么东西，同样搜索打断点，如下图所示 `_5DD` 就是 `data` 值，传过来的。

![11](https://cdn.itbob.cn/img/article/057/11.png)

往上跟栈，可以看到 `_Ug0` 里面有个 `track` 参数，这明显就是轨迹了，同样最后的结果经过了 AES 加密。

![12](https://cdn.itbob.cn/img/article/057/12.png)

再往上跟，可以看到 `_Ug0` 由三个参数组成，`x` 是水平滑动的距离，`track` 是轨迹，`p` 是定值。

![13](https://cdn.itbob.cn/img/article/057/13.png)
...
![14](https://cdn.itbob.cn/img/article/057/14.png)
...
![15](https://cdn.itbob.cn/img/article/057/15.png)
