...
tags: 
    ...
    - 爬虫
categories: 验证码逆向实战
feature: https://cdn.itbob.cn/img/thumbnail/captcha7.png
...
![01](https://cdn.itbob.cn/img/article/064/01.png)

完整流程：

![02](https://cdn.itbob.cn/img/article/064/02.jpg)

## 三代抓包情况

`register-fullpage` 会返回 `challenge` 和 `gt` 值，后期有用到。

![03](https://cdn.itbob.cn/img/article/064/03.png)

`get.php` 会返回 `c` 和 `s`，同样后期会用到，这个请求同样需要 `w` 值，在某验的点选滑块里，这个请求可以不要 `w` 值，但在三代无感里，必须得要，不然最后是会验证失败的，可能不带 `w` 请求，返回的 `s` 是个假的值，导致最后验证失败。

![04](https://cdn.itbob.cn/img/article/064/04.png)

`ajax.php` 验证是否成功，同样需要 `w` 值，成功则会返回一个 `validate`。

![05](https://cdn.itbob.cn/img/article/064/05.png)
...
![06](https://cdn.itbob.cn/img/article/064/06.png)
...
![07](https://cdn.itbob.cn/img/article/064/07.png)

同样需要注意的是上图中 `this[$_CHCFe(1393)]()` 也是个随机字符串，会遇到多次，但整体一次验证下来都是相同的值，不然验证会失败，点选、滑块等一系列都一样的。

![08](https://cdn.itbob.cn/img/article/064/08.png)

![09](https://cdn.itbob.cn/img/article/064/09.png)

然后是 o 的值，这里和以前一样，跟进去看仍然是 AES 加密。` t[$_CFAGw(1393)]()` 依旧是随机字符串。

![10](https://cdn.itbob.cn/img/article/064/10.png)

中间这一串 `pe[$_CFAFP(416)](t[$_CFAGw(353)])`，也没啥特别的，除了 `gt` 和 `challenge`，其他都是定值，最后面那一串 `i` 的值，实测写死、置空都行。

![11](https://cdn.itbob.cn/img/article/064/11.png)

然后继续跟进 `i`，把前面 `o` 的值转换成一个大的字符串，这里也不是很复杂，直接扣代码即可：

![12](https://cdn.itbob.cn/img/article/064/12.png)

![13](https://cdn.itbob.cn/img/article/064/13.png)
...
![17](https://cdn.itbob.cn/img/article/064/17.png)

`i[$_CGBGa(1415)]` 这里就是将每个变量和值挨个相加，组成一个大的字符串。

![18](https://cdn.itbob.cn/img/article/064/18.png)

然后到 r 这里，就多了一个 `captcha_token`，然后用 `{}` 将所有数据包了起来。

![19](https://cdn.itbob.cn/img/article/064/19.png)

这些数据我们格式化一下，大致结构如下图所示：

![20](https://cdn.itbob.cn/img/article/064/20.png)
...
![21](https://cdn.itbob.cn/img/article/064/21.png)

## 三代结果验证

![22](https://cdn.itbob.cn/img/article/064/22.png)

## 四代抓包情况

`load` 接口返回值如下：

![23](https://cdn.itbob.cn/img/article/064/23.png)
...
![24](https://cdn.itbob.cn/img/article/064/24.png)
...
![25](https://cdn.itbob.cn/img/article/064/25.png)
...
![26](https://cdn.itbob.cn/img/article/064/26.png)
...
![27](https://cdn.itbob.cn/img/article/064/27.png)
...
![28](https://cdn.itbob.cn/img/article/064/28.png)

i 定义在第 11702 行，跟进到 `d[$_DIEIq(103)]` 方法中，i 为 16 位随机数：

![29](https://cdn.itbob.cn/img/article/064/29.png)

e 参数内容如下：

![30](https://cdn.itbob.cn/img/article/064/30.png)
...
![31](https://cdn.itbob.cn/img/article/064/31.png)
...
![32](https://cdn.itbob.cn/img/article/064/32.png)

## 四代结果验证

![33](https://cdn.itbob.cn/img/article/064/33.png)
